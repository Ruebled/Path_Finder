...
ftxui::Color Color_cell[] = {
    ftxui::Color::Black,    ftxui::Color::Cyan,        ftxui::Color::Red,
    ftxui::Color::GrayDark, ftxui::Color::SandyBrown,  ftxui::Color::DarkGreen,
    ftxui::Color::DarkBlue, ftxui::Color::DarkOrange3, ftxui::Color::GrayLight,
};
...
ftxui::Canvas matrix_to_canvas(Grid grid, ftxui::Screen& screen) {
  ...
  for (int r = 0; r < rows; r++) {
    for (int c = 0; c <= cols; c++) {
      canvas.DrawPointLine(c * cell_dim_x + cell_dim_x, 0,
                           c * cell_dim_x + cell_dim_x, rows * cell_dim_y,
                           ftxui::Color::White);
    }
    canvas.DrawPointLine(0, r * cell_dim_y + cell_dim_y, cols * cell_dim_x,
                         r * cell_dim_y + cell_dim_y, ftxui::Color::White);
  }
  ...
  for (int r = 0; r < rows; r++) {
    for (int c = 0; c < cols; c++) {
      ...
      for (int l = 0; l <= cell_dim_y - 2; l++) {
        canvas.DrawPointLine(c * cell_dim_x + 2, r * cell_dim_y + 1 + l,
                             c * cell_dim_x + cell_dim_x - 2,
                             r * cell_dim_y + 1 + l, cell_color);
      }
    }
  }
