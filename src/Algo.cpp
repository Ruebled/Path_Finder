...
void BreathFirstSearch(Matrix& matrix) {
  ...
  std::priority_queue<CElement, std::vector<CElement>, std::greater<CElement>>
      neigh;
  ...
  while (!neigh.empty()) {
    ...
    for (int i = 0; i < near.size(); i++) {
      ...
      //
      double stat_cost = cost_stats[current] + (diagonal ? 0.4 : 0) +
                         get_cell_priority(matrix[near[i].y][near[i].x]);

      if (cost_so_far.find(near[i]) == cost_so_far.end() ||
          new_cost < cost_so_far[near[i]]) {
        if (cost_so_far.find(near[i]) == cost_so_far.end()) {
          if (matrix[near[i].y][near[i].x] != Type(end)) {
            matrix[near[i].y][near[i].x] =
                Type(visited) + matrix[near[i].y][near[i].x];
          }
        }
    ...
    while (current != start_point) {
      ...
      if (matrix[current.y][current.x] != Type(start)) {
        matrix[current.y][current.x] =
            matrix[current.y][current.x] - Type(visited) + Type(path);
      }
      distance_path++;
      // Path delay
      usleep(15 * 1000);
  ...
  while (!neigh.empty()) {
    ...
    for (int i = 0; i < near.size(); i++) {
      ...
      double new_cost = cost_so_far[current] +
                        (get_cell_priority(matrix[near[i].y][near[i].x] % 10)) +
                        (diagonal ? 0.5 : 0);
...
std::vector<point> get_neighbors(Matrix& mat, point t_point) {
  ...
  for (int row = -1; row < 2; row++) {
    ...
    for (int col = -1; col < 2; col++) {
      int t_x = px + col;
      if (t_y < 0 || t_y >= height || t_x < 0 || t_x >= width)
        continue;

      if (mat[t_y][t_x] == Type(start) || mat[t_y][t_x] == Type(wall) ||
          mat[t_y][t_x] == Type(visited))
        continue;

      // check if it's a diagonal to be pushed back
      if ((t_y != py && t_x != px) && !diag_checked)
        continue;
