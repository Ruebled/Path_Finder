...
int main(int argc, const char* argv[]) {
  ...
  auto grid_renderer =
      Renderer([&] { return canvas(matrix_to_canvas(grid, screen)); });
  ...
  auto grid_with_mouse = CatchEvent(grid_renderer, [&](Event e) {
    if (e.is_mouse()) {
      ...
      grid.on_mouse_event(row_pixel, col_pixel, mouse.button == Mouse::Left,
  ...
  auto toShorterFloat = [](float time, int len) {
    ...
    std::string real_time_string_trunc(real_time_string.begin(),
  ...
  ftxui::Element instructions_en = ftxui::vbox({
      ftxui::paragraph("This Project is visual implementation of some of the "
                       "most known path finding algorithms.") |
          ftxui::center,
  ...
  // Think of this
  auto button_tile_style_wall =
      ButtonOption::Animated(Color_cell[Type(wall)], Color::Black,
                             Color::Default, Color_cell[Type(wall)]);
  auto button_tile_style_sand =
      ButtonOption::Animated(Color_cell[Type(sand)], Color::Black,
                             Color::Default, Color_cell[Type(sand)]);
  auto button_tile_style_woods =
      ButtonOption::Animated(Color_cell[Type(woods)], Color::Black,
                             Color::Default, Color_cell[Type(woods)]);
  auto button_tile_style_water =
      ButtonOption::Animated(Color_cell[Type(water)], Color::Black,
                             Color::Default, Color_cell[Type(water)]);
  auto button_tile_style_mountains =
      ButtonOption::Animated(Color_cell[Type(mountains)], Color::Black,
                             Color::Default, Color_cell[Type(mountains)]);

  auto tiles_selector_container = Container::Vertical({
      Button(
          &types_of_tiles[0],
          [&] {
            grid.choose_tile(Type(wall));
            depth = 0;
          },
          &button_tile_style_wall) |
          center,
      Button(
          &types_of_tiles[1],
          [&] {
            grid.choose_tile(Type(sand));
            depth = 0;
          },
          &button_tile_style_sand) |
          center,
      Button(
          &types_of_tiles[2],
          [&] {
            grid.choose_tile(Type(woods));
            depth = 0;
          },
          &button_tile_style_woods) |
          center,
      Button(
          &types_of_tiles[3],
          [&] {
            grid.choose_tile(Type(water));
            depth = 0;
          },
          &button_tile_style_water) |
          center,
      Button(
          &types_of_tiles[4],
          [&] {
            grid.choose_tile(Type(mountains));
            depth = 0;
          },
          &button_tile_style_mountains) |
          center,
  });
  ...
  std::vector<std::string> help_text = {
      "This app implement some popular pathfinding algorithms in an "
      "interactive way.",

      "Drawing the map",
      "Using *rigth click* an window with clickable elements(types of tiles "
      "for choosing) will appear at the center of the screen",
      ...
      "     *Clear all* - clears all the tiles but the *start* and *end* "
      "points",
      "Map - is used to retrieve the maps saved before",

      "Path finding process",
      "After presssing the *Start* button, with a defined delay, the tiles "
      "checked by the algorithm will be colored in _gray_.",
      "When the path is found the algorithms stop checking any other tiles, "
      "and start to draw backwards the path found, then reverse visited tiles "
      "the way checked",
      "Then *real time* and the *CPU time* that the algorithm spent is "
  ...
  auto clear_button_style = ButtonOption::Animated(
  ...
  auto main_renderer = Renderer(main_container, [&] {
    ...
    if (depth == 2) {
      document = dbox({
          document,
          help_window_renderer->Render() | clear_under |
              size(HEIGHT, EQUAL, 39) | size(WIDTH, EQUAL, 80) | center,
      });
    }
    ...
    if (depth == 4) {
      document = dbox({
          document,
          shortcut_window_renderer->Render() | clear_under |
              size(HEIGHT, EQUAL, 17) | size(WIDTH, EQUAL, 60) | center,
      });
    }
